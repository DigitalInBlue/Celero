...
def run_git_bytes(project_root, *args):
	"""Like run_git, but returns raw stdout so file content is not translated."""
	try:
		result = subprocess.run(
			["git", *args],
			cwd=str(project_root),
...
def sha256_tracked_file(project_root, relative_path, warnings):
	"""SHA256 of a tracked file's canonical repository content.

	The bytes on disk are deliberately not hashed.  A Windows checkout with
	core.autocrlf materializes CRLF where the repository stores LF, so hashing
	the working tree yields a different digest on Windows than on Linux for a
	file nobody has touched -- which makes the committed SBOM unverifiable in CI.
	The blob git records is the same on every platform and is what a source
	archive of the repository contains.

	Falls back to the file on disk when git cannot answer, since an unpacked
	source archive has no repository and there the file is the distribution.
	"""
	path = project_root / relative_path
	blob = run_git_bytes(project_root, "cat-file", "blob", "HEAD:{}".format(relative_path))
	if blob is None:
		return sha256_file(path)

	# A local edit would otherwise be published under HEAD's digest.
	if path.read_bytes().replace(b"\r\n", b"\n") != blob.replace(b"\r\n", b"\n"):
		warnings.append(
			"{} differs from HEAD; the SBOM records the committed content, not the working "
			"copy. Commit the change before regenerating.".format(relative_path)
		)
...
def collect_vendored_packages(project_root, warnings):
	...
	for component in VENDORED_COMPONENTS:
		...
		package = {
			...
			"checksums": [
				{
					"algorithm": "SHA256",
					"checksumValue": sha256_tracked_file(project_root, component["path"], warnings),
				}
			],
		}
		packages.append(package)
	...
	document = {
		...
		"creationInfo": {
			...
			"comment": (
				...
				"headers are declared explicitly and hashed from their canonical repository content."
			),
		},
...
def generate_source_document(project_root, version, reference, commit, timestamp, vcpkg_paths, warnings):
	...
	for package in collect_vendored_packages(project_root, warnings):
		packages.append(package)
		relationships.append(
			{
				"spdxElementId": "SPDXRef-Package-celero",
